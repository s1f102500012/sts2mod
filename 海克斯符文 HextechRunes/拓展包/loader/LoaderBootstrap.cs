...
{
	private const string ModId = "HextechRunesSponsorPack";
	private const string RealDllName = "HextechRunesSponsorPack.dll";
	private const string VariantManifestName = "hextech-runes-sponsor-pack-variants.manifest";
	private const string CompatTargetMarkerName = "compat-target.txt";
	private const string CompatTargetMetadataKey = "HextechSponsorCompatibilityTarget";
	...
	public static void Initialize()
	{
		...
		{
			Log.Error("[HextechRunesSponsorPack.Loader] Could not resolve loader directory.");
			return;
		}
		...
		{
			Log.Error($"[HextechRunesSponsorPack.Loader] Missing lib directory: {libRoot}");
			return;
		}
		...
		{
			Log.Error(
				$"[HextechRunesSponsorPack.Loader] No valid variant under {libRoot} " +
				$"for host {host.ReleaseLabel ?? host.Numeric?.ToString() ?? "unknown"}.");
			return;
		}

		Log.Info(
			$"[HextechRunesSponsorPack.Loader] Host version label={host.ReleaseLabel ?? "<none>"} " +
		...
		{
			Log.Error(
				$"[HextechRunesSponsorPack.Loader] Failed to load or initialize " +
		...
		VariantCandidate variant)
	{
			...
			StringComparison.Ordinal))
		{
			throw new BadImageFormatException(
				$"Variant assembly identity is {assembly.GetName().Name}, expected HextechRunesSponsorPack.");
		}
	...
	private static void InstallReflectionBridge()
	{
		...
		new Harmony("Natsuki.HextechRunesSponsorPack.Loader.ReflectionBridge").Patch(
			getter,
	...
	private static IEnumerable<Type> GetLoadableTypes(Assembly assembly)
	{
		...
		{
			Log.Warn(
				$"[HextechRunesSponsorPack.Loader] Partial type load for " +
	...
	private static void AssociateVariantAssemblyWithGame(Assembly assembly)
	{
		...
		if (AssociateAssemblyWithModMethod != null)
		{
			...
			{
				Log.Warn(
					$"[HextechRunesSponsorPack.Loader] AssociateAssemblyWithMod failed: " +
		...
		Log.Warn(
			"[HextechRunesSponsorPack.Loader] Could not associate the selected variant " +
	...
	private static void OnLegacyModDetected(Mod mod)
	{
		...
		Log.Info(
			$"[HextechRunesSponsorPack.Loader] Associated variant " +
		...
		Version? host)
	{
		...
		{
			Log.Warn(
				"[HextechRunesSponsorPack.Loader] Host version is unknown; " +
		...
		string libRoot)
	{
		...
		if (!File.Exists(path))
		{
			Log.Error($"[HextechRunesSponsorPack.Loader] Missing variant manifest: {path}");
			return [];
		}
		...
		{
			Log.Error(
				$"[HextechRunesSponsorPack.Loader] Failed to read variant manifest: {exception}");
			return [];
		}
		...
		{
			Log.Error(
				$"[HextechRunesSponsorPack.Loader] Variant manifest contains no variants: {path}");
			return [];
		}
		...
		BundleVariantEntry entry)
	{
		...
		{
			Log.Error(
				$"[HextechRunesSponsorPack.Loader] Ignoring invalid target " +
				$"'{entry.CompatTarget}'.");
			return null;
		}
		...
		{
			Log.Error(
				$"[HextechRunesSponsorPack.Loader] Ignoring invalid variant directory " +
				$"'{relativeDirectory}'.");
			return null;
		}
		...
		{
			Log.Error(
				$"[HextechRunesSponsorPack.Loader] Ignoring variant with missing or " +
		...
		{
			Log.Error(
				$"[HextechRunesSponsorPack.Loader] Ignoring unexpected assembly " +
				$"'{assemblyName}'.");
			return null;
		}
		...
		{
			Log.Error(
				$"[HextechRunesSponsorPack.Loader] Ignoring missing or hash-mismatched " +
				$"variant: {dllPath}");
			return null;
		}
