	...
	internal static void EnsureHarmonyRuntimeDependenciesVisible()
	{
		...
		lock (Gate)
		{
			...
			try
			{
				...
				{
					Log.Warn(
						"[HextechRunesSponsorPack.Loader] Could not expose libgcc_s.so.1 globally; " +
						"Harmony patches may fail to initialize on native Linux.");
					return;
				}

				Log.Info(
					"[HextechRunesSponsorPack.Loader] Exposed libgcc_s.so.1 to Harmony's native helper.");
			}
			catch (Exception exception)
			{
				Log.Warn(
					$"[HextechRunesSponsorPack.Loader] Failed to expose libgcc_s.so.1 globally: {exception.Message}");
			}
		}
	}
