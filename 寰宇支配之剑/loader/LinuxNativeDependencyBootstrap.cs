	...
	internal static void EnsureHarmonyRuntimeDependenciesVisible()
	{
		...
		lock (Gate)
		{
			...
			try
			{
				...
				{
					Log.Warn(
						"[UniversalDominionSword.Loader] Could not expose libgcc_s.so.1 globally; " +
						"Harmony patches may fail to initialize on native Linux.");
					return;
				}

				Log.Info(
					"[UniversalDominionSword.Loader] Exposed libgcc_s.so.1 to Harmony's native helper.");
			}
			catch (Exception exception)
			{
				Log.Warn(
					$"[UniversalDominionSword.Loader] Failed to expose libgcc_s.so.1 globally: {exception.Message}");
			}
		}
	}
