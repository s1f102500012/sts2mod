...
public sealed class RepeatableCompositeEnchantment : EnchantmentModel
{
	...
	private string? SavedEnchantmentsJson
	{
		...
		set
		{
			...
			{
				Amount = 0;
				ModEntry.DebugLog("Save", "Loaded empty composite enchantment payload.");
				return;
			}
			...
			{
				Amount = 0;
				ModEntry.DebugLog("Save", "Composite enchantment payload deserialized to null.");
				return;
			}
			...
			ModEntry.DebugLog("Save", $"Loaded {_innerEnchantments.Count} inner enchantments into composite: {DescribeInnerEnchantments()}.");
		}
	}
	...
	public EnchantmentModel ImportExistingEnchantment(EnchantmentModel enchantment)
	{
		...
		{
			ModEntry.DebugLog("Composite", $"Rebinding imported enchantment {enchantment.Id.Entry} from {ModEntry.DescribeCard(enchantment.Card)} to {ModEntry.DescribeCard(Card)}.");
		...
		Amount = _innerEnchantments.Count;
		RefreshCompositeStatus();
		ModEntry.DebugLog("Composite", $"Imported existing enchantment {enchantment.Id.Entry} into {ModEntry.DescribeCard(Card)}. Current={DescribeInnerEnchantments()}.");
		return enchantment;
	}
	...
	public EnchantmentModel AddOrStackEnchantment(EnchantmentModel enchantment, decimal amount, bool refreshConsumedStacks)
	{
		...
		if (existing != null)
		{
			ModEntry.DebugLog("Composite", $"Stacking {enchantment.Id.Entry} on {ModEntry.DescribeCard(Card)} by {amount}. Existing amount={existing.Amount}, status={existing.Status}.");
		...
		RefreshCompositeStatus();
		ModEntry.DebugLog("Composite", $"Added new inner enchantment {enchantment.Id.Entry} to {ModEntry.DescribeCard(Card)}. Current={DescribeInnerEnchantments()}.");
		return enchantment;
	}
	...
	public override async Task AfterPlayerTurnStart(PlayerChoiceContext choiceContext, Player player)
	{
		...
		foreach (EnchantmentModel enchantment in _innerEnchantments)
		{
			if (enchantment is Imbued)
			{
				...
				{
					ModEntry.DebugLog("Imbued", $"Auto-playing imbued composite card {ModEntry.DescribeCard(compositeCard)} at player turn start.");
	...
	private Task HandleGoopyAfterCardPlayed(Goopy goopy, CardPlay cardPlay)
	{
		...
		if (goopy.Card.DeckVersion?.Enchantment is RepeatableCompositeEnchantment deckVersionComposite)
		{
