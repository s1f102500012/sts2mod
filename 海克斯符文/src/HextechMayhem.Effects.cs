...
internal sealed partial class HextechMayhemModifier
{
    ...
    private decimal CantTouchThisBufferStacks => ShouldScaleMonsterHexForMultiplayer ? MultiplayerPlayerCount * 2m : 2m;
    ...
    private async Task ApplyPersistentMonsterHexes(Creature creature)
    {
        ...
        {
            int maxHpGain = (int)Math.Floor(creature.MaxHp * 0.4m);
            if (maxHpGain > 0)
            {
    ...
    {
        float baseScale = HasActiveMonsterHex(MonsterHexKind.Goliath) ? 1.4f : 1f;
    ...
    {
        if (creature.Side != CombatSide.Enemy || !HasActiveMonsterHex(MonsterHexKind.GlassCannon))
        {
            return amount;
        }
