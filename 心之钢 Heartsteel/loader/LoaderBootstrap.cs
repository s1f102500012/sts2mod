...
namespace Heartsteel.Loader;

[ModInitializer(nameof(Initialize))]
public static class LoaderBootstrap
{
	private const string ModId = "Heartsteel";
	private const string RealDllName = "Heartsteel.dll";
	private const string VariantManifestName = "heartsteel-variants.manifest";
	private const string CompatTargetMarkerName = "compat-target.txt";
	private const string CompatTargetMetadataKey = "HeartsteelCompatibilityTarget";
	...
	public static void Initialize()
	{
		...
		{
			Log.Error("[Heartsteel.Loader] Could not resolve loader directory.");
			return;
		}
		...
		{
			Log.Error($"[Heartsteel.Loader] Missing lib directory: {libRoot}");
			return;
		}
		...
		{
			Log.Error(
				$"[Heartsteel.Loader] No compatible valid variant under {libRoot} " +
				$"for host {host.ReleaseLabel ?? host.Numeric?.ToString() ?? "unknown"}.");
			return;
		}

		Log.Info(
			$"[Heartsteel.Loader] Host version label={host.ReleaseLabel ?? "<none>"} " +
		...
		{
			Log.Error(
				$"[Heartsteel.Loader] Failed to load or initialize " +
	...
	private static void ValidateVariantAssembly(Assembly assembly, VariantCandidate variant)
	{
			...
			StringComparison.Ordinal))
		{
			throw new BadImageFormatException(
				$"Variant assembly identity is {identity ?? "<missing>"}, expected Heartsteel.");
		}
	...
	private static void InstallReflectionBridge()
	{
		...
		new Harmony("Natsuki.Heartsteel.Loader.ReflectionBridge").Patch(
			getter,
	...
	private static IEnumerable<Type> GetLoadableTypes(Assembly assembly)
	{
		...
		{
			Log.Warn(
				$"[Heartsteel.Loader] Partial type load for " +
	...
	private static void AssociateVariantAssemblyWithGame(Assembly assembly)
	{
		...
		if (AssociateAssemblyWithModMethod != null)
		{
			...
			{
				Log.Warn(
					$"[Heartsteel.Loader] AssociateAssemblyWithMod failed: " +
		...
		Log.Warn(
			"[Heartsteel.Loader] Could not associate the selected variant " +
	...
	private static void OnLegacyModDetected(Mod mod)
	{
		...
		Log.Info(
			$"[Heartsteel.Loader] Associated variant " +
	...
	private static void InvokeRealInitializer(Assembly assembly)
	{
		foreach (Type type in GetLoadableTypes(assembly))
		{
			...
			Log.Info(
				$"[Heartsteel.Loader] Invoked implementation initializer " +
		...
		Version? host)
	{
		...
		{
			Log.Warn(
				"[Heartsteel.Loader] Host version is unknown; " +
				"using the newest bundled variant as a best-effort fallback.");
			return variants[^1];
		}
		...
		string libRoot)
	{
		...
		if (!File.Exists(path))
		{
			throw new FileNotFoundException("Missing variant manifest.", path);
		}
		...
		if (manifest?.Variants == null || manifest.Variants.Count == 0)
		{
			throw new InvalidDataException(
				$"Variant manifest contains no variants: {path}");
		}
		...
		{
			variants.Add(CreateVariantCandidate(loaderDirectory, fullLibRoot, entry));
		}
		return variants;
	}
		...
		BundleVariantEntry entry)
	{
		...
		{
			throw new InvalidDataException(
				$"Invalid compatibility target '{entry.CompatTarget}'.");
		}
		...
		{
			throw new InvalidDataException(
				$"Unsafe variant directory '{relativeDirectory}'.");
		}
		...
		{
			throw new InvalidDataException(
				$"Missing or mismatched compatibility marker: {markerPath}");
		}
		...
		{
			throw new InvalidDataException(
				$"Unexpected variant assembly '{assemblyName}'.");
		}
		...
		{
			throw new InvalidDataException(
				$"SHA-256 mismatch for variant assembly: {dllPath}");
		}
